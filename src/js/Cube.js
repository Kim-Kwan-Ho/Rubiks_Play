...
class Cube {
	...
	constructor( game ) {
		...
		this.faceTexturePaths = {
			F: 'assets/images/cube-faces/front.png',
			B: 'assets/images/cube-faces/back.png',
			R: 'assets/images/cube-faces/right.png',
			L: 'assets/images/cube-faces/left.png',
			U: 'assets/images/cube-faces/up.png',
			D: 'assets/images/cube-faces/down.png',
		};
	...
	generateModel() {
		...
		this.positions.forEach( ( position, index ) => {
			...
			position.edges.forEach( edgeIndex => {
				...
				edge.name = name;
				edge.userData.grid = position.grid;
    ...
    this.edges.forEach( edge => this.updateEdgeMaterial( edge, colors ) );

	}

	loadFaceTextures() {

		const entries = Object.entries( this.faceTexturePaths );

		entries.forEach( ( [ faceName, texturePath ] ) => {
			...
			this.textureLoader.load(
				...
				texture => {
					...
					this.faceTextures[ faceName ] = texture;
					this.refreshEdgeMaterials();
	...
	refreshEdgeMaterials() {

		const colors = this.game.themes.getColors();
		if ( ! colors || ! Array.isArray( this.edges ) ) return;

		this.edges.forEach( edge => this.updateEdgeMaterial( edge, colors ) );

	}

	updateEdgeMaterial( edge, colors ) {

		if ( edge.material.map ) edge.material.map.dispose();

		const texture = this.createTileTexture( edge.name, edge.userData.grid );
		edge.material.color.setHex( texture ? 0xffffff : colors[ edge.name ] );
		edge.material.map = texture;
		edge.material.needsUpdate = true;

	}

	createTileTexture( faceName, gridPosition ) {

		if ( ! gridPosition ) return null;

		const baseTexture = this.faceTextures[ faceName ];
		if ( ! baseTexture ) return null;

		const texture = baseTexture.clone();
		const tile = this.getFaceTile( faceName, gridPosition );

		texture.repeat.set( 1 / this.size, 1 / this.size );
		texture.offset.set( tile.col / this.size, tile.row / this.size );
	...
	getFaceTile( faceName, gridPosition ) {
